...
echo "Building Alice AI Gateway Electron App..."
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o alice-ai-gateway
    cd electron
    npm install
    npm run build:mac
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o alice-ai-gateway.exe
    cd electron
    npm install
    npm run build:win
else
